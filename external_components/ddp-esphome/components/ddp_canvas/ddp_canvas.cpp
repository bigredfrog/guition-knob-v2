...
static inline uint16_t int_sqrt_u16(size_t v) {
  if (v == 0) return 0;
  size_t x = 1;
  while ((x + 1) * (x + 1) <= v) {
    ++x;
...
void DdpCanvas::on_data(size_t offset_px, const uint8_t* pixels,
  ...
  // Track the highest pixel index seen so we can infer square source size per frame.
  const size_t end_px = offset_px + pixel_count;
  size_t prev = frame_max_px_seen_.load(std::memory_order_relaxed);
  while (end_px > prev &&
         !frame_max_px_seen_.compare_exchange_weak(prev, end_px, std::memory_order_relaxed)) {
  }
...
void DdpCanvas::on_push() {
  ...
  // Infer source square side from received pixel count in this frame.
  const size_t frame_px = frame_max_px_seen_.exchange(0, std::memory_order_relaxed);
  uint16_t inferred_side = int_sqrt_u16(frame_px);
  if (inferred_side == 0) {
    inferred_side = (dst_w_ > 0) ? dst_w_ : w_;
  }
  ready_src_side_.store(inferred_side, std::memory_order_relaxed);
  ...
  if (back_buffers_ == 2) {
    if (have_ready_.exchange(false, std::memory_order_acquire)) {
      if (canvas_ && buf_px_ > 0 && front_buf_ && ready_buf_) {
        const uint16_t src_side = ready_src_side_.load(std::memory_order_relaxed);
        const bool needs_scale = (dst_w_ > 0 && dst_h_ > 0) &&
                                 (src_side > 0) &&
                                 (src_side != dst_w_ || src_side != dst_h_);
        if (needs_scale) {
          scale_square_nn_(ready_buf_, src_side, front_buf_, dst_w_, dst_h_);
        } else {
  ...
  if (back_buffers_ == 1 && need_copy_to_front_.exchange(false)) {
    if (front_buf_ && accum_buf_ && buf_px_) {
      ...
      const bool needs_scale = (dst_w_ > 0 && dst_h_ > 0) &&
                               ...
                               (src_side != dst_w_ || src_side != dst_h_);
      if (needs_scale) {
        scale_square_nn_(accum_buf_, src_side, front_buf_, dst_w_, dst_h_);
      } else {
...
void DdpCanvas::ensure_buffers_() {
  ...
  dst_w_ = static_cast<uint16_t>(draw_buf->header.w);
  dst_h_ = static_cast<uint16_t>(draw_buf->header.h);
...
void DdpCanvas::scale_square_nn_(const uint16_t* src, uint16_t src_side,
                                 uint16_t* dst, uint16_t dst_w, uint16_t dst_h) {
  if (!src || !dst || src_side == 0 || dst_w == 0 || dst_h == 0) {
    return;
  }

  // Nearest-neighbor stretch from square source to destination rectangle.
  for (uint16_t y = 0; y < dst_h; y++) {
    const uint32_t sy = (static_cast<uint32_t>(y) * src_side) / dst_h;
    const uint32_t src_row = sy * src_side;
    const uint32_t dst_row = static_cast<uint32_t>(y) * dst_w;
    for (uint16_t x = 0; x < dst_w; x++) {
      const uint32_t sx = (static_cast<uint32_t>(x) * src_side) / dst_w;
