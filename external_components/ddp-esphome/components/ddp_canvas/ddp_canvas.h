...
class DdpCanvas : public Component, public DdpRenderer {
 ...
 protected:
  ...
  std::atomic<size_t> frame_max_px_seen_{0};      // Highest pixel index received for current frame
  std::atomic<uint16_t> ready_src_side_{0};       // Source square side for ready frame
  uint16_t dst_w_{0};
  uint16_t dst_h_{0};

  bool bound_{false};

  // Scale a square source frame into the destination canvas buffer (nearest-neighbor).
  static void scale_square_nn_(const uint16_t* src, uint16_t src_side,
